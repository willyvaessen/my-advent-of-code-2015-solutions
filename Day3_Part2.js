...
//  Since we're seeing to routes, we need two sets of houses:
const all_houses = new Set();
const houses_santa = new Set();
const houses_robo_santa = new Set();

//  The rest will stay the same so far, I believe
const directions = input.split('');
let hor_santa = 0;
let ver_santa = 0;
let house = hor_santa + "x" + ver_santa;

//  Starting delivery
//  First house is at 0x0
hor_santa = 0;
ver_santa = 0;
let hor_robo_santa = 0;
let ver_robo_santa = 0;
houses_santa.add(house);    //  I'll add the first house to Santa's Set
// console.log(houses);

//  Now to split the instructions in two sets
const santa_directions = [];
const robo_santa_directions = [];

for (let i = 0; i < directions.length; i++) {
    if (i%2 === 0) {
        santa_directions.push(directions[i])
    } else {
        robo_santa_directions.push(directions[i])
    }
}

console.log(santa_directions);
console.log(robo_santa_directions);

//  From this point on, I will be iterating through the directions array:
for (let i = 0; i < santa_directions.length; i++) {
    // console.log(directions[i]);
    if (santa_directions[i] === "^") {
        // console.log("Moving NORTH");
        hor_santa += 1;
    } else if (santa_directions[i] === "v") {
        // console.log("Moving SOUTH");
        hor_santa -= 1;
    } else if (santa_directions[i] === ">") {
    ...
    house = hor_santa + "x" + ver_santa;
    // console.log("Next house is: " + house);
    houses_santa.add(house);
}

//  From this point on, I will be iterating through Robo-Santa's directions array:
for (let i = 0; i < robo_santa_directions.length; i++) {
    // console.log(directions[i]);
    if (robo_santa_directions[i] === "^") {
        // console.log("Moving NORTH");
        hor_robo_santa += 1;
    } else if (robo_santa_directions[i] === "v") {
        // console.log("Moving SOUTH");
        hor_robo_santa -= 1;
    } else if (robo_santa_directions[i] === ">") {
        // console.log("Moving EAST");
        ver_robo_santa += 1;
    } else if (robo_santa_directions[i] === "<") {
        // console.log("Moving WEST");
    ...
    house = hor_robo_santa + "x" + ver_robo_santa;
    // console.log("Next house is: " + house);
    houses_robo_santa.add(house);
}



console.log(houses_santa.size)
console.log(houses_robo_santa.size)

//  Finally: adding the two sets together;
for (const house of houses_santa) {
    // console.log(house);
    all_houses.add(house);
}

for (const house of houses_robo_santa) {
    // console.log(house)
    all_houses.add(house);
}

console.log("All houses visited at least once: " + all_houses.size);